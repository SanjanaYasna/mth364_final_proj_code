...
#in terms of B* from different values of k_mB
...
with open("NF1_kmb_irrev.csv", "w") as f:
    f.write("k_mb,A,B,trace,determinant,string_of_classification,stable\n")
f.close() 
...
#iterate between values of B from 0 to 1 with step 0.01
kmb_values =np.arange(0, 1.0, 0.01)

for kmb in kmb_values:
    #unique_solutions tracks solutions for a given value of B
    unique_solutions = [] 
    #gekko does grid search over solutions (9 x 9 ics)
    for i in values:
        for j in values:
            m = GEKKO(remote =False)   
            #all params below are the same except k_mB
            k_mB = kmb
    ...
    #now that we have the solutions for the vlaue of k_mB, compute stabilities of the solutions
    #iterate over solution
    
    for solution in unique_solutions:
        A = tensor(solution[0])
        B = tensor(solution[1])
        #jacobian with argnums specifying which paramters are arguments to take jacobian with respect to
        ...
        if det < 0:
            ...
            stable = True
        #write to csv
        with open("NF1_kmb_irrev.csv", "a") as f:
            #cnovert kmb to number
            kmb = float(k_mB)
            #get trace
            trace = trace[0]
            det = det[0]    
            f.write(f"{kmb},{A},{B},{trace},{det},{classification},{stable}\n")
        f.close()
        print(f"k_mb: {kmb}, A: {A}, B: {B}, trace: {trace}, determinant: {det}, classification: {classification}, stable: {stable}")